...
def judul_laporan_bulanan(jenis, nama_bulan, tahun):
    if jenis == "BK":
        return f"LAPORAN PERMOHONAN BUKA KEMBALI MASUK BULAN {nama_bulan.upper()} {tahun} DARI HUBUNGAN LANGGANAN"
    return f"LAPORAN PERMOHONAN MASUK BULAN {nama_bulan.upper()} {tahun} DARI HUBUNGAN LANGGANAN"


@app.route("/permohonan/laporan/bulanan")
def permohonan_laporan_bulanan():
    db = get_db()
    tahun_list = [
        r["t"] for r in db.execute(
            """SELECT DISTINCT strftime('%Y', tanggal_permohonan) t FROM permohonan
    ...
    tahun = request.args.get("tahun", "") or (tahun_list[0] if tahun_list else datetime.now().strftime("%Y"))
    bulan = request.args.get("bulan", "") or datetime.now().strftime("%m")
    jenis = request.args.get("jenis", "SIB")
    ...
    return render_template(
        "permohonan_laporan_bulanan.html", tahun_list=tahun_list, tahun=tahun, bulan=bulan, jenis=jenis,
        nama_bulan_list=list(enumerate(NAMA_BULAN, start=1)), rows=rows,
        judul=judul_laporan_bulanan(jenis, nama_bulan, tahun),
    )


@app.route("/permohonan/laporan/bulanan/unduh")
def permohonan_laporan_bulanan_unduh():
    db = get_db()
    tahun = request.args.get("tahun", "")
    bulan = request.args.get("bulan", "")
    jenis = request.args.get("jenis", "SIB")
    ...
    headers = ["No", "Nama Pelanggan", "Alamat", "Kelurahan", "Kecamatan", "No. SPK",
               "Ke Perencana", "Survey", "Kembali Ke Hublang", "Petugas Survey", "Keterangan"]
    for col, h in enumerate(headers, start=1):
    ...
    for i, p in enumerate(rows, start=1):
        ws.cell(row=r, column=1, value=i)
        ws.cell(row=r, column=2, value=p["nama_pelanggan"])
        ws.cell(row=r, column=3, value=p["lokasi"])
        ws.cell(row=r, column=4, value=p["kelurahan"])
        ws.cell(row=r, column=5, value=p["kecamatan"])
        ws.cell(row=r, column=6, value=i)  # No. SPK -- sementara samain sama nomor urut, lihat catatan
        ws.cell(row=r, column=7, value=p["tanggal_permohonan"])
        ws.cell(row=r, column=8, value=p["tanggal_survey"] or "")
        ws.cell(row=r, column=9, value=p["tanggal_kembali_hublang"] or "")
        ws.cell(row=r, column=10, value=p["petugas_survey"] or "")
        ws.cell(row=r, column=11, value=p["keterangan"] or "")
        r += 1

    widths = [5, 22, 26, 14, 14, 8, 12, 12, 14, 14, 20]
    ...
    return send_file(
        buf, as_attachment=True,
        download_name=f"laporan_permohonan_{jenis}_{tahun}-{bulan}.xlsx",
        mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    )
